...
export function ModuleManager({
  ...
  function submit(enabled: boolean) {
    void run(
      ...
      { success: enabled ? t.moduleManager.toastEnabled : t.moduleManager.toastDisabled },
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-border/60 p-4">
        <h3 className="mb-3 text-sm font-semibold">{t.moduleManager.setAccess}</h3>
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="space-y-1">
            <Label htmlFor="mod-scope">{t.moduleManager.scopeLabel}</Label>
            <Select id="mod-scope" value={scope} onChange={(e) => setScope(e.target.value as "platform" | "tenant")}>
              <option value="platform">{t.moduleManager.scopePlatform}</option>
              <option value="tenant">{t.moduleManager.scopeTenant}</option>
            </Select>
          </div>
          {scope === "tenant" ? (
            <div className="space-y-1">
              <Label htmlFor="mod-org">{t.common.tenant}</Label>
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          {t.moduleManager.hint}
        </p>
      </div>

      <section className="space-y-3">
        <h3 className="text-sm font-semibold">{t.moduleManager.currentFlags}</h3>
        {moduleFlags.length === 0 ? (
          <EmptyState title={t.moduleManager.emptyTitle} description={t.moduleManager.emptyDescription} />
        ) : (
          <div className="overflow-x-auto rounded-xl border border-border/60">
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-left text-xs uppercase text-muted-foreground">
                <tr>
                  <th className="px-4 py-2.5">{t.moduleManager.colModule}</th>
                  <th className="px-4 py-2.5">{t.moduleManager.colScope}</th>
                  <th className="px-4 py-2.5">{t.moduleManager.colState}</th>
                </tr>
              </thead>
