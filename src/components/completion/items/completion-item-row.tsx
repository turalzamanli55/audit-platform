...
import type { CompletionItemView } from "@/lib/completion/completion-workspace-view";
import { Button, Input } from "@/components/ui";
import { WorkspaceNoticeBanner, WorkspacePanel, WorkspaceStatusBadge } from "@/components/workspace";

type CompletionItemRowLabels = {
  unassigned: string;
...
type CompletionItemRowProps = {
  packageId: string;
  packageVersion: number;
  item: CompletionItemView;
  isArchived: boolean;
  canReview: boolean;
  labels: CompletionItemRowLabels;
};

export function CompletionItemRow({
  packageId,
  packageVersion,
  item,
  isArchived,
  ...
  const showActions =
    canReview &&
    !isArchived &&
    ["pending", "under_review", "returned"].includes(item.itemStatus);
  const showReopen = canReview && !isArchived && item.itemStatus === "resolved";
  ...
  return (
    <div className="space-y-3 px-5 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            {labels.itemTypes[item.itemType] ?? item.itemType}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <WorkspaceStatusBadge
            label={labels.itemStatuses[item.itemStatus] ?? item.itemStatus}
            variant={
      ...
      {item.returnNotes && item.itemStatus === "returned" ? (
