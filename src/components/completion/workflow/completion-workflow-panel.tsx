...
type CompletionWorkflowPanelProps = {
  review: CompletionWorkspaceView;
  labels: CompletionWorkflowLabels;
  canSubmit?: boolean;
  canReview?: boolean;
  canApprove?: boolean;
};

export function CompletionWorkflowPanel({
  review,
  labels,
  canSubmit = true,
  canReview = true,
  canApprove = true,
}: CompletionWorkflowPanelProps) {
  ...
  const submit = () => {
    startTransition(async () => {
      setError(null);
      const result = await submitCompletionAction({
        packageId: review.id,
  ...
  const sendBack = () => {
    startTransition(async () => {
      setError(null);
      const result = await returnCompletionAction({
        packageId: review.id,
        version: review.version,
  ...
  const approve = () => {
    startTransition(async () => {
      setError(null);
      const result = await approveCompletionAction({
        packageId: review.id,
        version: review.version,
      });
      if (!result.success) {
...
export function useCompletionWorkflowHandlers(review: CompletionWorkspaceView) {
  return useMemo(
    () => ({
      onSubmit: () =>
        submitCompletionAction({ packageId: review.id, version: review.version }),
      onReturn: (notes: string | null) =>
        returnCompletionAction({ packageId: review.id, version: review.version, notes }),
      onApprove: () =>
        approveCompletionAction({ packageId: review.id, version: review.version }),
    }),
