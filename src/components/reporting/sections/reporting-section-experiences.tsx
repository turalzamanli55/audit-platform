...
import { ReportSectionRow } from "@/components/reporting/items/reporting-item-row";
import { ReportingWorkspaceSectionShell } from "@/components/reporting/workspace/reporting-workspace-section-shell";
import { ReportingWorkspaceReadonlyNotice } from "@/components/reporting/workspace/reporting-workspace-states";
import {
  archiveReportingAction,
  archiveReportCommentAction,
  commentReportingAction,
  restoreReportingAction,
...
function useWorkspaceOrCreate(props: GateProps) {
  const { review, fieldworkStarted, fieldworkSubstantiallyComplete } = useReportingWorkspace();
...
function ReportSectionsSection(
  props: BaseProps & {
...
export function ExecutiveSummaryExperience(
...
export function FinancialStatementsExperience(
  props: BaseProps & { itemLabels: ItemLabels },
) {
  const workspace = useWorkspaceOrCreate(props);
  if (typeof workspace !== "object" || !("items" in workspace)) return workspace;
  ...
  const items = workspace.items.filter(
    (item: ReportSectionView) => item.sectionType === "audit_findings" && Boolean(item.severity),
  );
...
export function OutstandingItemsPendingExperience(
  ...
  const items = workspace.items.filter((item: ReportSectionView) =>
...
export function ChecklistResolvedExperience(
  ...
  const items = workspace.items.filter((item: ReportSectionView) => item.sectionStatus === "resolved");
...
function ReviewNotesSection(
  ...
  const addNote = () => {
    ...
    startTransition(async () => {
      clearError();
      const result = await commentReportingAction({
        packageId: workspace.id,
  ...
  const saveEdit = (comment: ReportCommentView) => {
    startTransition(async () => {
      clearError();
      const result = await updateReportCommentAction({
        packageId: workspace.id,
        packageVersion: workspace.version,
  ...
  const archiveNote = (comment: ReportCommentView) => {
    startTransition(async () => {
      clearError();
      const result = await archiveReportCommentAction({
        packageId: workspace.id,
        packageVersion: workspace.version,
        commentId: comment.id,
        commentVersion: comment.version,
  ...
  const restoreNote = (comment: ReportCommentView) => {
    startTransition(async () => {
      clearError();
      const result = await restoreReportCommentAction({
        packageId: workspace.id,
        packageVersion: workspace.version,
        commentId: comment.id,
        commentVersion: comment.version,
  ...
  const toggleResolved = (comment: ReportCommentView) => {
    startTransition(async () => {
      clearError();
      const action = comment.resolvedAt ? unresolveReportCommentAction : resolveReportCommentAction;
  ...
  const renderNote = (comment: ReportCommentView, depth = 0) => (
...
export function ReportCommentsExperience(
  props: BaseProps & {
    commentTypeLabels: Record<string, string>;
    noteLabels: NoteLabels;
  },
...
export function ReportVersionsExperience(
  props: BaseProps & {
    versionLabels: {
      latestBadge: string;
  ...
  const restore = () => {
    ...
    startTransition(async () => {
      clearError();
      const result = await restoreReportVersionAction({
        packageId: workspace.id,
        version: workspace.version,
        versionId: restoreId,
      });
  ...
  return (
    ...
    >
      ...
      <WorkspaceTable<ReportVersionView>
        columns={[
          {
            id: "version",
...
export function ReportingHistoryExperience(
  props: BaseProps & {
          ...
          >
            ...
            {actions.map((action) => (
              <option key={action} value={action}>
                {formatReportActivityAction(action, props.historyLabels.actions)}
              </option>
            ))}
          </select>
        </label>
        ...
        columns={[
          {
            ...
            cell: (entry) =>
              formatReportActivityAction(entry.action, props.historyLabels.actions),
          },
          {
            id: "actor",
          ...
          },
          {
            id: "summary",
            header: props.labels.columns?.summary ?? props.labels.description,
            cell: (entry) => formatReportActivitySummary(entry.summary),
          },
          {
            id: "created",
...
export function ReportingSettingsExperience(
  props: BaseProps & {
    settingsLabels: {
      archiveAction: string;
  ...
  const archive = () => {
    startTransition(async () => {
      clearError();
      const result = await archiveReportingAction({
        packageId: workspace.id,
        version: workspace.version,
      });
      if (!result.success) {
  ...
  const restore = () => {
    startTransition(async () => {
      clearError();
      const result = await restoreReportingAction({
        packageId: workspace.id,
        version: workspace.version,
      });
      if (!result.success) {
  ...
  return (
    ...
    >
      ...
      {!props.canArchive ? (
        <ReportingWorkspaceReadonlyNotice message={props.settingsLabels.readOnlyNotice} />
      ) : null}
