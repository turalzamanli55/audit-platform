...
export function LocaleSwitcher({
  ...
  const trigger =
    responsive ? (
      <Button
        variant="ghost"
        className="group h-9 w-9 shrink-0 gap-1.5 p-0 font-normal text-muted-foreground data-[state=open]:text-foreground sm:h-10 sm:w-10 lg:h-10 lg:w-auto lg:px-2.5"
        aria-label={label}
      >
        ...
        <IconChevronDown className="hidden shrink-0 transition-transform duration-200 group-data-[state=open]:rotate-180 lg:block" />
      </Button>
    ) : compact ? (
    ...
    <DropdownMenu className={className} trigger={trigger}>
      ...
      {locales.map((item) => (
        ...
        >
          ...
          {item.code === locale ? <IconCheck width={16} height={16} className="text-primary" /> : null}
        </DropdownMenuItem>
      ))}
    </DropdownMenu>
  );
