...
import { FinancialStatementSectionRow } from "@/components/financial-statements/items/financial-statements-item-row";
...
import { FinancialStatementsWorkspaceReadonlyNotice } from "@/components/financial-statements/workspace/financial-statements-workspace-states";
import {
  ...
  restoreFinancialStatementVersionAction,
...
import {
  formatFinancialStatementActivityAction,
  formatFinancialStatementActivitySummary,
...
function useWorkspaceOrCreate(props: GateProps) {
  const { review, fieldworkStarted, fieldworkSubstantiallyComplete } = useFinancialStatementsWorkspace();
...
function FinancialStatementSectionsSection(
  props: BaseProps & {
...
export function BalanceSheetExperience(
  ...
  const items = workspace.items.filter(
    (item: FinancialStatementSectionView) => item.sectionType === "balance_sheet",
  );

  return (
    <FinancialStatementSectionsSection
      {...props}
      items={items}
      headingId="balance-sheet"
    />
  );
}

export function IncomeStatementExperience(
  props: BaseProps & { itemLabels: ItemLabels },
) {
  const workspace = useWorkspaceOrCreate(props);
  if (typeof workspace !== "object" || !("items" in workspace)) return workspace;
  ...
  const items = workspace.items.filter(
    (item: FinancialStatementSectionView) => item.sectionType === "income_statement",
  );
  ...
  return (
    <FinancialStatementSectionsSection
      {...props}
      items={items}
      headingId="income-statement"
    />
  );
}

export function CashFlowStatementExperience(
  props: BaseProps & { itemLabels: ItemLabels },
) {
  const workspace = useWorkspaceOrCreate(props);
  if (typeof workspace !== "object" || !("items" in workspace)) return workspace;
  ...
  const items = workspace.items.filter(
    (item: FinancialStatementSectionView) => item.sectionType === "cash_flow_statement",
  );
  ...
  return (
    <FinancialStatementSectionsSection
      {...props}
      items={items}
      headingId="cash-flow-statement"
    />
  );
}

export function NotesLinksExperience(
  props: BaseProps & { itemLabels: ItemLabels },
) {
  const workspace = useWorkspaceOrCreate(props);
  if (typeof workspace !== "object" || !("items" in workspace)) return workspace;
  ...
  const items = workspace.items.filter(
    (item: FinancialStatementSectionView) => item.sectionType === "notes_links",
  );
  ...
  return (
    <FinancialStatementSectionsSection
      {...props}
      items={items}
      headingId="notes-links"
    />
  );
}

export function CrossReferencesExperience(
  props: BaseProps & { itemLabels: ItemLabels },
) {
  const workspace = useWorkspaceOrCreate(props);
  if (typeof workspace !== "object" || !("items" in workspace)) return workspace;
  ...
  const items = workspace.items.filter(
    (item: FinancialStatementSectionView) => item.sectionType === "cross_references",
  );
  ...
  return (
    <FinancialStatementSectionsSection
      {...props}
      items={items}
      headingId="cross-references"
    />
  );
}

function LegacyCrossReferencesExperience(
  props: BaseProps & { itemLabels: ItemLabels },
) {
  const workspace = useWorkspaceOrCreate(props);
  if (typeof workspace !== "object" || !("items" in workspace)) return workspace;
...
function ReviewNotesSection(
  props: BaseProps & {
    notes: FinancialStatementCommentView[];
    commentType: "reviewer" | "financial_statement" | "internal";
    headingId: string;
  ...
  const childrenByParent = useMemo(() => {
    const map = new Map<string, FinancialStatementCommentView[]>();
  ...
  const addNote = () => {
    ...
    startTransition(async () => {
      clearError();
      const result = await commentFinancialStatementsAction({
        packageId: workspace.id,
  ...
  const saveEdit = (comment: FinancialStatementCommentView) => {
    startTransition(async () => {
      clearError();
      const result = await updateFinancialStatementCommentAction({
        packageId: workspace.id,
        packageVersion: workspace.version,
  ...
  const archiveNote = (comment: FinancialStatementCommentView) => {
    startTransition(async () => {
      clearError();
      const result = await archiveFinancialStatementCommentAction({
        packageId: workspace.id,
        packageVersion: workspace.version,
        commentId: comment.id,
        commentVersion: comment.version,
  ...
  const restoreNote = (comment: FinancialStatementCommentView) => {
    startTransition(async () => {
      clearError();
      const result = await restoreFinancialStatementCommentAction({
        packageId: workspace.id,
        packageVersion: workspace.version,
        commentId: comment.id,
        commentVersion: comment.version,
  ...
  const toggleResolved = (comment: FinancialStatementCommentView) => {
    startTransition(async () => {
      clearError();
      const action = comment.resolvedAt ? unresolveFinancialStatementCommentAction : resolveFinancialStatementCommentAction;
  ...
  const renderNote = (comment: FinancialStatementCommentView, depth = 0) => (
  ...
  return (
    ...
    >
      ...
      {mutable ? (
        ...
        <FinancialStatementsWorkspaceReadonlyNotice
          message={props.archivedReadOnlyLabel ?? props.workspaceLabels.archivedDescription}
        />
      )}
...
export function ChangesInEquityExperience(
  props: BaseProps & { itemLabels: ItemLabels },
) {
  const workspace = useWorkspaceOrCreate(props);
  if (typeof workspace !== "object" || !("items" in workspace)) return workspace;
  ...
  const items = workspace.items.filter(
    (item: FinancialStatementSectionView) => item.sectionType === "changes_in_equity",
  );
  ...
  return (
    <FinancialStatementSectionsSection
      {...props}
      items={items}
      headingId="changes-in-equity"
    />
  );
}

export function FinancialStatementCommentsExperience(
  props: BaseProps & {
    commentTypeLabels: Record<string, string>;
    noteLabels: NoteLabels;
  },
  ...
  return (
    <ReviewNotesSection
      ...
      headingId="financialStatements-comments"
    />
  );
}

export function FinancialStatementVersionsExperience(
  props: BaseProps & {
    versionLabels: {
      latestBadge: string;
  ...
  const restore = () => {
    ...
    startTransition(async () => {
      clearError();
      const result = await restoreFinancialStatementVersionAction({
        packageId: workspace.id,
        version: workspace.version,
        versionId: restoreId,
      });
  ...
  return (
    ...
    >
      ...
      <WorkspaceTable<FinancialStatementVersionView>
        columns={[
          {
            id: "version",
...
export function FinancialStatementsHistoryExperience(
  props: BaseProps & {
    activity: FinancialStatementActivityView;
    historyLabels: {
      versionLabel: string;
      updatedLabel: string;
          ...
          >
            ...
            {actions.map((action) => (
              <option key={action} value={action}>
                {formatFinancialStatementActivityAction(action, props.historyLabels.actions)}
              </option>
            ))}
          </select>
        </label>
        ...
        columns={[
          {
            ...
            cell: (entry) =>
              formatFinancialStatementActivityAction(entry.action, props.historyLabels.actions),
          },
          {
            id: "actor",
          ...
          },
          {
            id: "summary",
            header: props.labels.columns?.summary ?? props.labels.description,
            cell: (entry) => formatFinancialStatementActivitySummary(entry.summary, entry.action),
          },
          {
            id: "created",
...
export function FinancialStatementsSettingsExperience(
  props: BaseProps & {
    settingsLabels: {
      archiveAction: string;
  ...
  const archive = () => {
    startTransition(async () => {
      clearError();
      const result = await archiveFinancialStatementsAction({
        packageId: workspace.id,
        version: workspace.version,
      });
      if (!result.success) {
  ...
  const restore = () => {
    startTransition(async () => {
      clearError();
      const result = await restoreFinancialStatementsAction({
        packageId: workspace.id,
        version: workspace.version,
      });
      if (!result.success) {
  ...
  return (
    ...
    >
      ...
      {!props.canArchive ? (
        <FinancialStatementsWorkspaceReadonlyNotice message={props.settingsLabels.readOnlyNotice} />
      ) : null}
...
export const FinancialStatementsCommentsExperience = FinancialStatementCommentsExperience;
export const FinancialStatementsVersionsExperience = FinancialStatementVersionsExperience;
