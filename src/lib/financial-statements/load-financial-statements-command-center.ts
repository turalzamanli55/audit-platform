...
import type {
  FinancialStatementsCommandCenterData,
  ReviewCommandKpi,
  FinancialStatementsWorkflowStep,
} from "@/types/financial-statements-command-center";

type CommandCenterLabels = Dictionary["financialStatements"]["workspace"]["commandCenter"];
...
function resolveCurrentWorkflowStep(review: FinancialStatementsWorkspaceView): string {
  if (review.packageStatus === "published") return "published";
  ...
  if (review.packageStatus === "prepared") return "prepared";
...
function buildWorkflowSteps(
...
): { steps: FinancialStatementsWorkflowStep[]; current: string } {
  ...
  const steps: FinancialStatementsWorkflowStep[] = WORKFLOW_STEP_IDS.map((id, index) => ({
    id,
...
): FinancialStatementsCommandCenterData["recentChanges"] {
  const events: FinancialStatementsCommandCenterData["recentChanges"] = [];
...
function mapItem(
  review: FinancialStatementsWorkspaceView,
  item: FinancialStatementsWorkspaceView["items"][number],
  locale: string,
  sectionTypeLabels: Record<string, string>,
  sectionStatusLabels: Record<string, string>,
): FinancialStatementsCommandCenterData["reviewQueue"][number] {
  const base = `/${locale}/app/engagements/${review.engagementSlug}/financial-statements`;
  return {
    id: item.id,
    title: item.title,
    ...
    href: item.href ?? `${base}/financial-statements-type`,
  };
}

export function loadFinancialStatementsCommandCenter(input: {
  locale: Locale;
  ...
  const mapComment = (
    comment: FinancialStatementsWorkspaceView["comments"][number],
  ): FinancialStatementsCommandCenterData["comments"][number] => ({
    id: comment.id,
