...
export async function loadReportingDashboardMetrics(): Promise<ReportingDashboardMetrics | null> {
  try {
    ...
    for (const engagement of engagements) {
      ...
      const pkg = (result.data ?? null) as ReportingPackageRow | null;
      if (!pkg) continue;

      if (["submitted", "under_review"].includes(pkg.package_status)) pendingReports += 1;
      ...
      const activityResult = await db
        ...
        .select("id")
        .eq("reporting_package_id", pkg.id)
        .is("deleted_at", null)
        .maybeSingle();
