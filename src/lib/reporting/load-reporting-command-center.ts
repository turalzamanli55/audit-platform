...
import type {
  ReportingCommandCenterData,
  ReviewCommandKpi,
  ReportingWorkflowStep,
} from "@/types/reporting-command-center";

type CommandCenterLabels = Dictionary["reporting"]["workspace"]["commandCenter"];
...
function resolveCurrentWorkflowStep(review: ReportingWorkspaceView): string {
...
function buildWorkflowSteps(
...
): { steps: ReportingWorkflowStep[]; current: string } {
  ...
  const steps: ReportingWorkflowStep[] = WORKFLOW_STEP_IDS.map((id, index) => ({
    id,
...
): ReportingCommandCenterData["recentChanges"] {
  const events: ReportingCommandCenterData["recentChanges"] = [];
...
function mapItem(
  review: ReportingWorkspaceView,
  item: ReportingWorkspaceView["items"][number],
  locale: string,
  sectionTypeLabels: Record<string, string>,
  sectionStatusLabels: Record<string, string>,
): ReportingCommandCenterData["reviewQueue"][number] {
...
export function loadReportingCommandCenter(input: {
  locale: Locale;
  ...
  const mapComment = (
    comment: ReportingWorkspaceView["comments"][number],
  ): ReportingCommandCenterData["comments"][number] => ({
    id: comment.id,
