...
export async function loadReviewDashboardMetrics(): Promise<ReviewDashboardMetrics | null> {
  try {
    ...
    for (const engagement of engagements) {
      const packageResult = await db
        ...
        .select(
          "id, package_status, pending_count, returned_count, resolved_count, open_findings_count",
        )
        .eq("engagement_id", engagement.id)
        .is("deleted_at", null)
        .maybeSingle();

      const pkg = (packageResult as { data: ReviewPackageRow | null }).data;
      if (!pkg) continue;
