...
export type OpinionDashboardMetrics = {
...
export async function loadOpinionDashboardMetrics(): Promise<OpinionDashboardMetrics | null> {
  try {
    ...
    for (const engagement of engagements) {
      ...
      const pkg = (result.data ?? null) as OpinionPackageRow | null;
      if (!pkg) continue;

      if (["submitted", "under_review"].includes(pkg.package_status)) pendingOpinions += 1;
