import { LOCKED_OPINION_STATUSES } from "@/constants/opinion";
...
export type OpinionWorkflowTarget = {
  package_status: string;
  deleted_at?: string | null;
};

export type OpinionSubmitCheck = {
  pendingCount: number;
  returnedCount: number;
  totalItems: number;
};

export type ComputeOpinionProgressInput = {
  totalItems: number;
...
export function isOpinionApproved(
  reportingPackage: Pick<OpinionWorkflowTarget, "package_status">,
): boolean {
  return reportingPackage.package_status === "approved";
}

export function assertOpinionApprovedForExport(
  reportingPackage: Pick<OpinionWorkflowTarget, "package_status"> | null,
): void {
  if (!reportingPackage || !isOpinionApproved(reportingPackage)) {
    throw new ValidationError(
      "Export cannot begin until the audit opinion is approved.",
    );
  }
}
...
export function assertPackageEditable(reportingPackage: OpinionWorkflowTarget): void {
  if (
    LOCKED_OPINION_STATUSES.includes(
      reportingPackage.package_status as (typeof LOCKED_OPINION_STATUSES)[number],
    ) ||
    reportingPackage.deleted_at
  ) {
    throw new ValidationError("Approved or archived opinion packages are read-only.");
  }
}
...
export function assertCanSubmit(
  ...
  if (
    ...
    throw new ValidationError("Only draft or returned opinion packages can be submitted.");
  }
}
...
export function assertCanReturn(
  ...
  if (
    ...
    throw new ValidationError("Only submitted opinion packages can be returned.");
  }
}
...
export function assertCanApprove(
  ...
  if (
    ...
    throw new ValidationError("Only submitted opinion packages can be approved.");
  }
}

export function assertSubmitPrerequisites(check: OpinionSubmitCheck): void {
  if (check.totalItems === 0) {
    throw new ValidationError("At least one opinion section must exist before submission.");
  }
  if (check.pendingCount > 0) {
    throw new ValidationError("All opinion sections must be resolved before submission.");
  }
  if (check.returnedCount > 0) {
    throw new ValidationError("Returned opinion sections must be addressed before submission.");
  }
}

export function computeOpinionProgress(input: ComputeOpinionProgressInput): number {
