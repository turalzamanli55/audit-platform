...
export type ArchiveEngagementActionInput = {
  engagementId: string;
  version: number;
  archiveReason?: string | null;
};

export type ArchiveEngagementActionResult = {
  engagementId: string;
  version: number;
  status: string;
};
...
export const archiveEngagementAction = defineEngagementAction<
  ArchiveEngagementActionInput,
  ArchiveEngagementActionResult
>({ module: "engagement.archive" }, ENGAGEMENT_PERMISSIONS.ARCHIVE, async (input, context) => {
  ...
  const engagement = await repository.archive(input.engagementId, input.version);

  const requestHeaders = await headers();
  await emitAuditEvent({
    action: AUDIT_ACTIONS.ENGAGEMENT_ARCHIVED,
