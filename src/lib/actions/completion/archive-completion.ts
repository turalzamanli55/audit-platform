...
import { validateArchiveCompletionInput } from "@/lib/completion/validation";
...
export const archiveCompletionAction = defineCompletionAction<
  ArchiveReviewActionInput,
  ArchiveReviewActionResult
>({ module: "review.archive" }, COMPLETION_PERMISSIONS.ARCHIVE, async (input, context) => {
  const validated = validateArchiveCompletionInput(input);
  ...
  await emitAuditEvent({
    action: AUDIT_ACTIONS.COMPLETION_ARCHIVED,
...
export const restoreCompletionAction = defineCompletionAction<
  { packageId: string; version: number },
  ArchiveReviewActionResult
>({ module: "review.restore" }, COMPLETION_PERMISSIONS.ARCHIVE, async (input, context) => {
  const validated = validateArchiveCompletionInput(input);
  ...
  const requestHeaders = await headers();
  await emitAuditEvent({
    action: AUDIT_ACTIONS.COMPLETION_RESTORED,
    resourceType: AUDIT_RESOURCE_TYPE,
    resourceId: updated.id,
    organizationId: context.organizationId,
    workspaceId: context.workspaceId,
