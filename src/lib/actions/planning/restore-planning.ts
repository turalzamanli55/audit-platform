...
export type RestorePlanningActionInput = {
  planId: string;
  version: number;
};

export type RestorePlanningActionResult = {
  planId: string;
  version: number;
  status: string;
};
...
export const restorePlanningAction = definePlanningAction<
  RestorePlanningActionInput,
  RestorePlanningActionResult
>({ module: "planning.restore" }, PLANNING_PERMISSIONS.ARCHIVE, async (input, context) => {
  if (!input.planId) {
  ...
  const plan = await repository.restore(input.planId, input.version);

  const requestHeaders = await headers();
  await emitAuditEvent({
    action: AUDIT_ACTIONS.PLANNING_RESTORED,
