...
export type FinancialReportingActionContext = ActionContext & {
  userId: string;
...
type Handler<TInput, TOutput> = (
  input: TInput,
  context: FinancialReportingActionContext,
) => Promise<TOutput>;

export function createFinancialReportingAction<TInput, TOutput>(
...
export function createFrRepositoryContext(
  userId: string,
  organizationId: string,
  workspaceId: string,
): RepositoryContext {
