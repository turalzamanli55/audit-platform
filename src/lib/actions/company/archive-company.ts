...
export type ArchiveCompanyActionInput = {
  companyId: string;
  version: number;
  archiveReason?: string | null;
};

export type ArchiveCompanyActionResult = {
  companyId: string;
  version: number;
  status: string;
};
...
export const archiveCompanyAction = createCompanyAction<
  ArchiveCompanyActionInput,
  ArchiveCompanyActionResult
>({ module: "company.archive" }, COMPANY_PERMISSIONS.ADMINISTER, async (input, context) => {
  if (!input.companyId) {
  ...
  const company = await repository.archive(input.companyId, input.version);
