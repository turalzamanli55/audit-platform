...
import {
  validateAddOpinionCommentInput,
...
export const commentOpinionAction = defineCompletionAction<
  import("@/lib/opinion/validation").AddOpinionCommentInput,
  { commentId: string; version: number }
>({ module: "opinion.comment.add" }, OPINION_PERMISSIONS.COMMENT, async (input, context) => {
  const validated = validateAddOpinionCommentInput(input);
  ...
  const comment = await reviewRepository.addComment({
    ...
    opinionSectionId: validated.opinionSectionId,
...
export const resolveOpinionSectionAction = defineCompletionAction<
  import("@/lib/opinion/validation").OpinionSectionMutationInput,
  { itemId: string; packageVersion: number; itemVersion: number }
>({ module: "opinion.item.resolve" }, OPINION_PERMISSIONS.REVIEW, async (input, context) => {
...
export const returnOpinionSectionAction = defineCompletionAction<
  import("@/lib/opinion/validation").OpinionSectionMutationInput,
  { itemId: string; packageVersion: number; itemVersion: number }
>({ module: "opinion.item.return" }, OPINION_PERMISSIONS.REVIEW, async (input, context) => {
  const validated = validateOpinionSectionMutationInput(input);
