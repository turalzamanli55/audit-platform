import { ReportingHistoryExperience } from "@/components/reporting";
...
import { loadReportActivityCached } from "@/lib/reporting/load-reporting-activity";
...
export default async function Page({ params }: PageProps) {
  ...
  const activityResult = await loadReportActivityCached(slug);
  const activity = activityResult.ok ? activityResult.activity : { entries: [] };

  return (
    <ReportingHistoryExperience
      {...common}
      labels={dictionary.reporting.history}
      activity={activity}
    />
  );
}
