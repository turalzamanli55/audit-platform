import { OpinionHistoryExperience } from "@/components/opinion";
...
import { loadOpinionActivityCached } from "@/lib/opinion/load-opinion-activity";
...
export default async function Page({ params }: PageProps) {
  ...
  const activityResult = await loadOpinionActivityCached(slug);
  const activity = activityResult.ok ? activityResult.activity : { entries: [] };

  return (
    <OpinionHistoryExperience
      {...common}
      labels={dictionary.opinion.history}
      activity={activity}
    />
  );
}
